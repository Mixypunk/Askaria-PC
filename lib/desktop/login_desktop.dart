...
class _LoginDesktopState extends State<LoginDesktop> {
  ...
  Future<void> _login() async {
    ...
    if (customServer.isNotEmpty) {
      await sl<SwingApiService>().saveUrl(customServer);
    }

    try {
      final ok = await sl<SwingApiService>().login(_u.text.trim(), _p.text);
      if (!mounted) return;
      if (ok) {
      ...
      setState(() {
        _loading = false;
        _err = e is Exception ? e.toString().replaceFirst('Exception: ', '') : e.toString();
      });
    }
  }
